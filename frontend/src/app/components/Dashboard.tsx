...
export function Dashboard() {
  ...
  // Find the most urgent thing to work on
  const next = getNextSubTask();

  // Basic time-based greeting for emotional design
  const hour = new Date().getHours();
  const greeting = hour < 12 ? 'Good morning' : hour < 17 ? 'Good afternoon' : 'Good evening';

  // Only show tasks that still have incomplete steps
  const ongoingTasks = tasks.filter(t => t.subTasks.some(st => !st.completed));

  // Quick math for the top progress bar
  ...
  return (
    <div className="max-w-md mx-auto px-5 pt-12 pb-4" style={{ fontFamily: "'DM Sans', 'Inter', sans-serif" }}>
      ...
      {/* "Up Next" Card - Reduces choice paralysis */}
      {next && (
        <motion.div
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <span
              ...
              style={{ backgroundColor: DIFFICULTY_COLORS[next.subTask.difficulty].bg, color: DIFFICULTY_COLORS[next.subTask.difficulty].text }}
            >
          ...
          <button
            ...
            className="w-full bg-primary text-primary-foreground rounded-xl py-3.5 flex items-center justify-center gap-2 shadow-[0_4px_14px_rgba(124,182,157,0.4)] active:scale-[0.98] transition-all"
          >
      ...
      {/* List of assignments user is currently working on */}
        ...
        <div className="space-y-3">
          {ongoingTasks.map((task, taskIdx) => {
            ...
            return (
              <motion.button
                key={task.id}
                initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + taskIdx * 0.08 }}
                onClick={() => navigate(`/assignment/${task.id}`)}
                className="w-full text-left bg-card rounded-2xl border border-border p-4 active:scale-[0.98] transition-all"
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      <span className="text-[12px] text-muted-foreground">{completedCount}/{task.subTasks.length} steps</span>
                    </div>
                  </div>
          ...
          {/* Empty state if user has finished everything */}
          {ongoingTasks.length === 0 && (
            <div className="text-center py-8">
              <p className="text-muted-foreground text-[14px]">No ongoing assignments</p>
              <button onClick={() => navigate('/new-task')} className="mt-3 text-primary text-[14px] hover:underline">
